...
export class B2StorageService implements IStorageService {
  ...
  private authInfo: B2AuthInfo | null = null;
  ...
  async listFiles(prefix?: string, maxFiles: number = 1000): Promise<StorageFile[]> {
    ...
    while (files.length < maxFiles) {
      ...
      files = files.concat(response.data.files.map((file: { 
        fileName: string;
        ...
        etag: file.fileId,
        isDirectory: file.fileName.endsWith('/')
      })));

      if (!response.data.nextFileName) break;
  ...
  getFileUrl(path: string, expiresIn: number = 3600): string {
    ...
    return `${this.authInfo.downloadUrl}/file/${this.bucket}/${path}?Authorization=${downloadAuth}`;
  }
